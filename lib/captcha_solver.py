...
class CaptchaSolver:
    ...
    def resolve(self, left_image, right_image, sign_image, negative_sign_right_image):
        ...
        if sign in {'+', '@', '4','*'}:
            ...
            right_number = self.pipe(negative_sign_right_image)[0]['generated_text']
            return self.math_operation(left_number, right_number, '-')
        else:
            unfixed_right_number = ''.join(char for char in self.pipe(right_image)[0]['generated_text'] if char.isdigit())
    ...
    def solve_captcha(self):
        ...
        dimensions = {'width': 27, 'height': 36, 'width_sign': 15, 'height_sign': 15, 'width_negative_sign': 18}
