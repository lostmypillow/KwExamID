...
# class CaptchaSolver:
#     def __init__(self, image_bytes):
#         self.image = cv2.imdecode(np.fromstring(image_bytes, np.uint8), cv2.IMREAD_COLOR)
#         self.kernel = np.ones((2, 2), np.uint8)
#         self.pipe = pipeline(
#             "image-to-text", model="microsoft/trocr-large-printed")
...
#     def resolve(self, left_image, right_image, sign_image, negative_sign_right_image):
#         sign = self.pipe(sign_image)[0]['generated_text']
#         left_number = self.pipe(left_image)[0]['generated_text']

#         if sign in {'+', '@', '4', '*'}:
#             right_number = self.pipe(right_image)[0]['generated_text']
#             return self.math_operation(left_number, right_number)
#         elif sign in {'-', '='}:
#             right_number = self.pipe(negative_sign_right_image)[
#                 0]['generated_text']
#             return self.math_operation(left_number, right_number, '-')
#         else:
#             unfixed_right_number = ''.join(char for char in self.pipe(right_image)[
#                                            0]['generated_text'] if char.isdigit())
#             return self.math_operation_for_both_signs(left_number, unfixed_right_number)

#     def solve_captcha(self):
#         positions = {'left': 5, 'right': 62,
#                      'sign': 39, 'negative_sign_right': 56}
#         dimensions = {'width': 27, 'height': 36, 'width_sign': 15,
#                       'height_sign': 15, 'width_negative_sign': 18}

#         left_image = self.image[6:36, positions['left']:positions['left']+27]
#         right_image = self.image[6:36,
#                                  positions['right']:positions['right']+24]
#         sign_image = self.image[10:25, positions['sign']                                :positions['sign']+dimensions['width_sign']]
#         negative_sign_right_image = self.image[7:27, positions['negative_sign_right']                                               :positions['negative_sign_right']+dimensions['width_negative_sign']]

#         left_enhanced = self.enhance_legibility(left_image)
#         right_enhanced = self.enhance_legibility(right_image)
#         negative_sign_right_enhanced = self.enhance_legibility(
#             negative_sign_right_image)

#         cv2.imwrite('left_number.jpg', left_enhanced)
#         cv2.imwrite('right_number.jpg', right_enhanced)
#         cv2.imwrite('sign.jpg', sign_image)
#         cv2.imwrite('negative_sign_right_number.jpg',
#                     negative_sign_right_enhanced)
